...
parser.add_argument('--model', default='pointnet2', help='Model name [default: model]')
parser.add_argument('--log_dir', default='pointnet2_all_', help='Log dir [default: log]')
...
Point_Util = os.path.join(BASE_DIR, 'utils', 'pointnet_util.py')
...
DATA_PATH = os.path.join("/home/vision/ware/zym/data")
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            # Get model and loss 
            pred, predn, end_points = MODEL.get_model(pointclouds_pl, is_training_pl, NUM_CLASSES, bn_decay=bn_decay)
            loss = MODEL.get_loss(pred, labels_pl, smpws_pl)
            lossa = MODEL.get_loss(predn, labels_pl, smpws_pl)
            lossall = loss + 0.2*lossa
            tf.summary.scalar('loss', loss)
            tf.summary.scalar('lossa', lossa)
...
def get_batch_wdp(dataset, idxs, start_idx, end_idx):
    ...
    for i in range(bsize):
        ...
        dropout_ratio = np.random.random()*0.975 # 0-0.875
