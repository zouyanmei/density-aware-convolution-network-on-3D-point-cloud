...
parser.add_argument('--data', type=str, default='/home/vision/ware/zym/data/SemanticKITTI/dataset/', help='Root for dataset')
...
parser.add_argument('--batch_size', type=int, default=24, help='Batch Size during training [default: 6]')
...
Point_Util = '../utils/' + 'pointnet_util.py'
...
os.system('cp semantic_kitti_dataset.py %s' %(LOG_DIR))
os.system('cp train_semantic_kitti.py %s' %(LOG_DIR))  # bkp of train procedure
...
TRAIN_DATASET = semantic_kitti_dataset.SemanticKittiDataset(root=DATA_PATH, sample_points=NUM_POINT, split='train', with_remission=FLAGS.with_remission)
print("Start loading validation data ...")
TEST_DATASET = semantic_kitti_dataset.SemanticKittiDataset(root=DATA_PATH, sample_points=NUM_POINT, split='valid', with_remission=FLAGS.with_remission)
print("Start loading whole scene validation data ...")
TEST_DATASET_WHOLE_SCENE = semantic_kitti_dataset.SemanticKittiDataset_whole(root=DATA_PATH, sample_points=NUM_POINT, split='valid', with_remission=FLAGS.with_remission)
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:0'):
            pointclouds_pl, labels_pl, smpws_pl = MODEL.placeholder_inputs(
                BATCH_SIZE, NUM_POINT, feature_channel=feature_channel)
            ...
            # Get model and loss
            pred, predn, end_points = MODEL.get_model(
                pointclouds_pl, is_training_pl, NUM_CLASSES, bn_decay=bn_decay, feature_channel=feature_channel)
            loss = MODEL.get_loss(pred, labels_pl, smpws_pl)
            lossa = MODEL.get_loss(predn, labels_pl, smpws_pl)
            lossall = loss + 0.5*lossa
            tf.summary.scalar('lossall', lossall)
            ...
            train_op = optimizer.minimize(lossall, global_step=batch)
        ...
        for epoch in range(MAX_EPOCH):
            ...
            #    eval_whole_scene_one_epoch(sess, ops, whole_test_writer)
            if iou > best_iou:
                best_iou = iou
            ...
            if epoch % 5 == 0:
                save_path = saver.save(
                    sess, os.path.join(LOG_DIR, "latest_model.ckpt"))
                log_string("Model saved in file: %s" % save_path)
            log_string('Best sample scan class avg mIOU is: %.5f' % best_iou)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in tqdm(range(num_batches),total=num_batches):
        ...
        summary, step, _, loss_val, pred_val = sess.run([ops['merged'], ops['step'],
                                                         ops['train_op'], ops['lossall'], ops['pred']], feed_dict=feed_dict)
    ...
    log_string('Trainingaccuracy: %f' % (total_correct / float(total_seen)))
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for batch_idx in tqdm(range(num_batches),total=num_batches):
        ...
        summary, step, loss_val, pred_val = sess.run([ops['merged'], ops['step'],
                                                      ops['lossall'], ops['pred']], feed_dict=feed_dict)
